...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = function (objid, graphConfig, penaltyData) {
	    ...
	    dataManager.fixDataParam(penaltyData);
	    ...
	    zr.showHint = function () {
	        ...
	        if (x + textLen / 2 > graphConfig.width) {
	            x = graphConfig.width - textLen / 2 - 30;
	        }
	        if( y - 80 < 0 ) {
	            y = 80;
	        }
...
/***/ function(module, exports, __webpack_require__) {
	...
	!(__WEBPACK_AMD_DEFINE_RESULT__ = function () {
	        ...
	        DataManager.prototype.fixDataParam = function (penaltyData) {
	            penaltyData.factorArr.sort(this.factorComparetor);
	        };

	        DataManager.prototype.factorComparetor = function (element1, element2) {
	            if (element1.level != element2.level) {
	                return element1.level - element2.level;
	            }
	            if( element1.countWay != element2.countWay ) {
	                return element1.countWay - element2.countWay;
	            }
	        ...
	        * 计算  preCount * （1 +/- currPercent ）
	        */
	        function getLevel1CountData(penaltyData, preCount, currPercent, factorConfig) {
	            var countData = {
	                level: 1,
	                preCount: preCount,
	            ...
	            //根据上个中点计算出下个中点
	            if (currPercent > factorConfig.changeMax) {
	                currPercent = factorConfig.changeMax;
	            }
	            if (currPercent < factorConfig.changeMin) {
	                currPercent = factorConfig.changeMin;
	            }
	            if (isAdd) {
	                countData.rangeHigh = (preCount + offset) * (1 + factorConfig.changeMax / 100);
	                countData.rangeLow = (preCount - offset) * (1 + factorConfig.changeMin / 100);
	                countData.count = preCount * (1 + currPercent / 100);
	            } else {
	                countData.rangeHigh = (preCount + offset) * (1 - factorConfig.changeMin / 100);
	                countData.rangeLow = (preCount - offset) * (1 - factorConfig.changeMax / 100);
	                countData.count = preCount * (1 - currPercent / 100);
	            }
	            fixCountData(penaltyData, countData);
	            if (isAdd) {
	                countData.percent = (countData.count / preCount - 1) * 100;
	            } else {
	                countData.percent = (1 - countData.count / preCount) * 100;
	            }
	            return countData;
	        }
	         ...
	         * 计算  preCount +/- baseCount * currPercent;
	         */
	        function getLevel2CountData(penaltyData, baseCount, preCount, currPercent, factorConfig) {
	            var countData = {
	                level: 2,
	                baseCount: baseCount,
	                preCount: preCount,
	            ...
	            var offset = penaltyData.rangeInfo.caliber / 2;
	            var isAdd = 1 == factorConfig.countWay || 'add' == factorConfig.countWay || 'ADD' == factorConfig.countWay;
	            //如果上次计算时，被迫被值为最大最小值，比例可能不在配置范围内，先尝试恢复比例到最近的正常范围进行计算
	            if (currPercent > factorConfig.changeMax) {
	                currPercent = factorConfig.changeMax;
	            }
	            if (currPercent < factorConfig.changeMin) {
	            ...
	            if (isAdd) {
	                countData.count = preCount + baseCount * currPercent / 100;
	                countData.rangeHigh = (preCount + offset) + baseCount * factorConfig.changeMax / 100;
	                countData.rangeLow = (preCount - offset) + baseCount * factorConfig.changeMin / 100;
	            } else {
	                countData.count = preCount - baseCount * currPercent / 100;
	                countData.rangeHigh = (preCount + offset) - baseCount * factorConfig.changeMin / 100;
	                countData.rangeLow = (preCount - offset) - baseCount * factorConfig.changeMax / 100;
	            }
	            //如果按照正常范围算出来的值越界了，值为最大最小值，并计算当前比例（此时当前比例可能不在范围内）
	            fixCountData(penaltyData, countData);
	            if (isAdd) {
	                countData.percent = (countData.count - countData.preCount) / countData.baseCount * 100;
	            } else {
	                countData.percent = (countData.preCount - countData.count) / countData.baseCount * 100;
	            }
	            return countData;
	        }
	        ...
	        function fixCountData(penaltyData, countData) {
	            ...
	            if (penaltyData.rangeInfo.rangeMin != null) {
	                ...
	                if (countData.count - caliber / 2 < min) {
	                    countData.count = min + caliber / 2;
	                }
	                if (countData.rangeHigh - caliber < min) {
	                    countData.rangeHigh = min + caliber;
	                }
	            }
	            if (penaltyData.rangeInfo.rangeMax != null) {
	                ...
	                if (countData.count + caliber / 2 > max) {
	                    countData.count = max - caliber / 2;
	                }
	                if (countData.rangeLow + caliber > max) {
	                    countData.rangeLow = max - caliber;
	                }
	            }
	        ...
	        DataManager.prototype.recountPercent = function (countData) {
	            var offset = this.penaltyData.rangeInfo.caliber / 2;
	            ...
	            if (countData.level == 1) {
	                if (isAdd) {
	                    countData.percent = (countData.count / countData.preCount - 1) * 100;
	                } else {
	                    countData.percent = (1 - countData.count / countData.preCount) * 100;
	                }
	            }
	        }
	        ...
	        DataManager.prototype.getCountDataArr = function (startPenalty, basePenalty, factorArr, redScrollBarArr) {
	            var countDataArr = [];
	            //计算红色坐标点、线、区域
	            for (var i = 0, preCount, lastRedScrollBar; i < factorArr.length; i++) {
	                ...
	                if (i == 0) {
	                    ...
	                    baseCount = scrollBarData.count;
	                    preCount = scrollBarData.count;
	                } else {
	                    ...
	                    if (1 == factorArr[i].level) {
	                        scrollBarData = getLevel1CountData(this.penaltyData, preCount, currPercent, factorArr[i]);
	                        baseCount = scrollBarData.count;
	                    } else {
	                        scrollBarData = getLevel2CountData(this.penaltyData, baseCount, preCount, currPercent, factorArr[i]);
	                    }
	                    preCount = scrollBarData.count;
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = function (graphConfig, penaltyData, zrender) {
	    ...
	    function getPointList(leftBar, rightBar) {
	        ...
	        var leftScrollBarTop = leftBar.scrollBarData.count + penaltyData.rangeInfo.caliber / 2;
	        var leftScrollBarBottom = leftBar.scrollBarData.count - penaltyData.rangeInfo.caliber / 2;
	        var rightScrollBarTop = rightBar.scrollBarData.count + penaltyData.rangeInfo.caliber / 2;
	        var rightScrollBarBottom = rightBar.scrollBarData.count - penaltyData.rangeInfo.caliber / 2;
	        var leftHY = dataManager.converPenalty2Y(leftScrollBarTop);
	        var leftLY = dataManager.converPenalty2Y(leftScrollBarBottom);

	        var rightHY = dataManager.converPenalty2Y(rightScrollBarTop);
	        if (rightBar.scrollBarData.rangeHigh != null) {
	            rightHY = dataManager.converPenalty2Y(rightBar.scrollBarData.rangeHigh);
	        }
	        var rightLY = dataManager.converPenalty2Y(rightScrollBarBottom);
	        if (rightBar.scrollBarData.rangeLow != null) {
	            rightLY = dataManager.converPenalty2Y(rightBar.scrollBarData.rangeLow);
	        }
	        return [[leftX, leftHY], [leftX, leftLY], [rightX, rightLY], [rightX, rightHY]];
	    }
	    ...
	    RedAreaGroup.prototype.addRedScrollBar = function (scrollBarData) {
	        ...
	        var barTopY = dataManager.converPenalty2Y(scrollBarData.count + penaltyData.rangeInfo.caliber / 2);
	        var barBottomY = dataManager.converPenalty2Y(scrollBarData.count - penaltyData.rangeInfo.caliber / 2);
	    ...
	    */
	    RedAreaGroup.prototype.updateRedScrollBar = function (scrollBarData) {
	        ...
	        //根据新数据绘制图像位置
	        var newTopYear = scrollBarData.count + penaltyData.rangeInfo.caliber / 2;
	        var newBottomYear = scrollBarData.count - penaltyData.rangeInfo.caliber / 2;
	        var barTopY = dataManager.converPenalty2Y(newTopYear);
	        var barBottomY = dataManager.converPenalty2Y(newBottomYear);
	        var x = dataManager.converIndex2X(scrollBarData.index);
	    ...
	    function scrollBarDrift(dx, dy) {
	        ...
	        var currBarTopY = dataManager.converPenalty2Y(currPenalty + yearOffset);
	        var currBarBottomY = dataManager.converPenalty2Y(currPenalty - yearOffset);
	    ...
	    function getShowPenaltyText(scrollBarGroup) {
	        var text;
	        var scrollBarData = scrollBarGroup.scrollBarData;
	        var sourceData = penaltyData.factorArr[scrollBarData.index];
	        text = sourceData.name;
	        // if (scrollBarData.percent) {
	        //     text += ' ';
	        //     var isAdd = (1 == sourceData.countWay || 'add' == sourceData.countWay || 'ADD' == sourceData.countWay);
	        //     text = text + (isAdd ? '加刑' : '减刑');
	        //     text = text + Math.round(scrollBarData.percent) + '%';
	        // }
	        text = '柱体中点：' + dataManager.getYMShowText(scrollBarData.count);
	        text = text + '\n' + '量刑建议区间：';
	        text += dataManager.getYMShowText(scrollBarData.count - penaltyData.rangeInfo.caliber / 2);
	        text += ' 到 ';
	        text += dataManager.getYMShowText(scrollBarData.count + penaltyData.rangeInfo.caliber / 2);
	    ...
	    var redAreaGroup = new RedAreaGroup({ 'id': 'redAreaGroup' });
	        ...
	        RANGE_0TO3_DETETION: {
	            ...
	            createRecText: function (penaltyData, dataResult) {
	                ...
	                if (middle > 6 && min < 6) {
	                    ...
	                    result.recText = createCommonRecText("有期徒刑", 6, middle + 0.5);
	                }
	                if (middle <= 6 && max > 6) {
	                ...
	                if (middle >= 1 && min < 1) {
	                    result.recText = createCommonRecText("拘役", 1, middle + 0.5);
	                }
	                return result;
	            }
	        },
	        ...
	        RANGE_0TO3_CONTROL: {
	            ...
	            createRecText: function (penaltyData, dataResult) {
	                ...
	                if (middle > 24 && min < 24) {
	                    ...
	                    result.recText = createCommonRecText("有期徒刑", 24, middle + 1.5);
	                }
	                if (min >= 24) {
	                    result.recText = createCommonRecText("有期徒刑", min, max);
	                }
	                ...
	                if (middle >= 3 && min < 3) {
	                    result.recText = createCommonRecText("管制", 3, middle + 1.5);
	                }
	                if (min >= 3 && max <= 24) {
	        ...
	        RANGE_0TO3_INC: {
	            ...
	            createRecText: function (penaltyData, dataResult) {
	                ...
	                if (middle <= 6 && max > 6) {
	                    ...
	                    result.recText = createCommonRecText("拘役", middle, 6);
	                }
	                if (middle > 6 && min < 6) {
	                    result.recText = createCommonRecText("有期徒刑", 6, middle + 6);
	                }
	                if (min >= 6) {
	                    result.recText = createCommonRecText("有期徒刑", min, max);
	                }
	        ...
	        */
	        RANGE_3TO10_INC: {
	            ...
	            createRecText: function (penaltyData, dataResult) {
	                ...
	                if (middle > 6 && min < 6) {
	                    result.hint = "计算结果低于有期徒刑最小范围，建议更换刑种为拘役或管制";
	                    result.recText = createCommonRecText("有期徒刑", 6, middle + 12);
	                }
	                return result;
	            }
	        },
